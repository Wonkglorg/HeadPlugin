...
public class HeadMenuGui extends InventoryGUI {
    ...
    public HeadMenuGui(Heads plugin, MenuProfile profile) {
        ...
        return new Button(icon) {
            ...
            public void onClick(InventoryClickEvent e) {
                new HeadMenuPage(plugin,
                        profile,
                        UtilityPlugin.manager().getConfigManager().getConfig(YML.HEAD_DATA.getFileName()),
                        "Heads",
                        null,
                        1).open();
            }
    ...
    private Button AdvancementsButton() {
        ...
        if (plugin.advancementApiExists()) {
            icon = new ItemBuilder(Material.NETHER_BRICK).setName(Message.color("Advancements")).build();
        } else {
            icon = new ItemBuilder(Material.BEDROCK).setName(Message.color(ChatColor.RED + "Advancements not installed")).build();
        }
    ...
    private Button CloseButton() {
        ItemStack icon = new ItemBuilder(Material.BARRIER).setName(Message.color("Close")).build();
        return new Button(icon) {
            @Override
            public void onClick(InventoryClickEvent e) {
                destroy();
